...
description: >
  ...
  produces a durable outcome for dsh-applications. Skip trivial or transient
  work.
...
use it for shipped application changes, validated milestones, compatibility or
...
- Run inside the `dsh-applications` git worktree with `docs/devlog/` present.
...
2. Keep normative docs current first. The devlog records the narrative and
   does not own the current application contract, architecture, or runbook.
